...
#ifndef LINUX
// Add "Openssl\lib" to Additional Library Directories
// Precompiled Version of OpenSSL from "https://slproweb.com/products/Win32OpenSSL.html" used
// Version: Win32 OpenSSL v1.1.0e
#pragma comment(lib, "libssl32MT.lib")       // wenn OpenSSL v1.0.2: libeay32.lib
...
Content_Type Smtp_Content_List[] =
{
	{ ".bmp", "image/bmp" },
	{ ".gif", "image/gif" },
	{ ".jpeg","image/jpeg" },
...
char* FindContentType(char* FileExt)
{
	for (int i = 0; i < sizeof(Smtp_Content_List) / sizeof(Smtp_Content_List[0]); ++i)
	{
	...
	try
	{
		...
		for (FileId = 0; FileId < Attachments.size(); FileId++)
		{
			...
			if (FileExt != NULL)
			{
				...
				strcat_s(SendBuf, BUFFER_SIZE, FindContentType(FileExt));
				strcat_s(SendBuf, BUFFER_SIZE, ";\r\n\tname=");
			}
			else
...
// Print Error Message of WinSocket
void CSmtp::GetWSALastErrorMsg() {
	wchar_t *s = NULL;
...
std::string ECSmtp::GetErrorText() const
{
	...
	case ECSmtp::WSA_STARTUP:
		return "Unable to initialise winsock2 (WSA)";
	case ECSmtp::WSA_VER:
		return "Wrong version of the winsock2 (WSA)";
	case ECSmtp::WSA_SEND:
		return "Function send() failed (WSA)";
	case ECSmtp::WSA_RECV:
		return "Function recv() failed (WSA)";
	case ECSmtp::WSA_CONNECT:
		return "Function connect() failed (WSA)";
	case ECSmtp::WSA_GETHOSTBY_NAME_ADDR:
		return "Unable to determine remote server (WSA)";
	case ECSmtp::WSA_INVALID_SOCKET:
		return "Invalid winsock2 socket (WSA)";
	case ECSmtp::WSA_HOSTNAME:
		return "Function hostname() failed (WSA)";
	case ECSmtp::WSA_IOCTLSOCKET:
		return "Function ioctlsocket() failed (WSA)";
	case ECSmtp::WSA_SELECT:
		return "Function select() failed (WSA)";
	...
	case ECSmtp::BAD_LOGIN_PASS:
		return "Bad login or password (535)";
	...
	case ECSmtp::RECONNECTION_EXCEED:
		return "The maximum hop count was exceeded for the message (446)";
	case ECSmtp::SENDING_TIMEOUT:
		return "Outgoing message timed out (447)";
	...
	case ECSmtp::LOCAL_ERROR:
		return "local error with the sending server - (Spam-Filter?) (471)";
	case ECSmtp::COMMAND_UNRECOGNIZED:
		return "Syntax error, command unrecognised (500)";
	case ECSmtp::ARGUMENT_ERROR:
		return "Syntax error in parameters or arguments (501)";
	case ECSmtp::COMMAND_NOT_IMPLEMENTED:
		return "Command not implemented (502)";
	case ECSmtp::BAD_SEQUENCE:
		return "Bad sequence of commands (503)";
	case ECSmtp::COMMAND_PARAMETER_NOT_IMPLEMENTED:
		return "Requested action not taken: mailbox unavailable (504)";
	case ECSmtp::BAD_EMAIL:
		return "Bad email address (510)";
	...
	default:
		return "Undefined error id (" + std::to_string((long long) static_cast<int>(ErrorCode)) + ")";
	}
}
...
void CSmtp::OpenSSLConnect()
{
	...
	while (1)
	{
		...
		default:
			FD_ZERO(&fdwrite);
			FD_ZERO(&fdread);
			/*Format: [pid]:error:[error code]:[library name]:[function name]:[reason string]:[file name]:[line]:[optional text message]*/
